...
PleaseInteger =  ['Пожалуйста ответь в целых числах. Если ответов несколько называй их через "и".', 'Прошу тебя, напиши ответ целыми числами. Если у тебя получилось два ответа, напиши их через "и"', 'Я принимаю ответ только в виде целых чисел. Если при решении задачи у тебя получилось два ответа, то выведи их через "и"']
...
HelpPhrases = ["В данном навыке потребуется решать математические задачи. После каждого ответа нужно выбирать: идти либо влево, либо вправо. С помощью кнопки или сказав \"Выход\" можно выйти из навыка. "]
...
RoomWelcome =  ['Чудесно', 'Прекрасно', 'Отлично', 'И так']
...
rooms_variants = {
    ...
    "Решение квадратных уравнений": ["Здесь тебе нужно найти корни квадратного уравнения и сказать ответ в целых числах, ответы разделяются союзом и. Если ответа нет скажи 'ответа нет'. Уравнение: ", rooms.quadraticEquationGenerator],
    ...
    if req.session_state["stage"] > 1:
        if (req.session_state["leftRight"]):
            try:
                ...
                if re.search("лев|прав",req.command.lower()):
                    ...
                    if image is None:
                        resp.set_text(f"{random.choice(RoomWelcome)}, ты оказался в комнате под названием '{name}'. {description}{exp}")
                    else:
                        resp.set_text(f"{random.choice(RoomWelcome)}, ты оказался в комнате под названием '{name}'. {description}")
                        resp.bigcard(image,f"{random.choice(RoomWelcome)}, ты оказался в комнате под названием '{name}'.", f"{description}{exp}" )
        ...
        if (req.session_state["think"]):
            ...
            try:
                ...
                resp.set_session_state(req.session_state)
                resp.set_text(random.choice(PleaseInteger))
                resp.set_buttons([{"title": "Выход", "hide": True}])
                return resp.dictionary
